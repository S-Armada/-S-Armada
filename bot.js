...
const log = (message, error = null) => {
    ...
    const prefix = `\x1b[36m[ S_Armada Obf Botz ]\x1b[0m`;
...
async function checkChannelMembership(ctx) {
    const channelId = "@S_Armada_Info"; // ID channel
    try {
...
const getSiuCalcrickObfuscationConfig = () => {
    const generateSiuCalcrickName = () => {
        ...
        return `Char和S-Armada无与伦比的帅气${randomPart}`;
    };
...
async function updateProgress(ctx, message, percentage, status) {
    ...
    const levelText = percentage === 100 ? "✅ Selesai" : `✨ ${status}`;
    try {
...
bot.start(async (ctx) => {
    ...
    const inlineKeyboard = [
        [
            { text: 'Bot 🛡', url: 'https://t.me/@s_armada_bot' },
                    { text: 'Owner ☘️', url: 'https://t.me/S_Armada' }
        ],
        [
            { text: 'Bot Info ✨', callback_data: 'infobot' },               
                    { text: 'Random ⚙️', callback_data: 'randommenu' }
        ]
    ];

    setTimeout(async () => {
        await ctx.replyWithPhoto("https://files.catbox.moe/plrqbs.jpg", {
            caption: `
Halo, saya bot obfuscator, di kembangkan oleh @S_Armada
🌱Selamat mencoba🌱
━━━━━━━━━━━━━━━━━━━━━
╭═─═〣 S-Armada 〣⪼
║ ✦ /enc <LEVEL> - STANDARD OBFUSCATION
│ ✦ /enceval <LEVEL> - EVALUATE & OBFUSCATE
║ ✦ /encchina - MANDARIN STYLE
│ ✦ /encarab - ARABIC STYLE
║ ✦ /encjapan - JAPANESE STYLE
│ ✦ /encinvis - INVISIBLE MODE
║ ✦ /encjapxab - JAPAN X ARAB
│ ✦ /encx - BASE 64
║ ✦ /encnebula - HARD DESIGN
│ ✦ /encnova - NOVA STYLE
║ ✦ /encsiu - SIU + CALCRICK 
│ ✦ /customenc <NAME> - CUSTOM DESIGN
║ ✦ /encmax <INTENSITY> - CUSTOM INTENSITY
│ ✦ /encstralth - STEALTH MODE
║ ✦ /encstrong - POWER FORTRESS
│ ✦ /encultra - ULTRA PROTECTION
╎ ✦ /deobfuscate - DECRYPT FILE
║ ✦ /encbig <MB> - MEGABYTE MODE
│ ✦ /encnew - ADVANCED LAYER
║ ✦ /encquantum QUANTUM FORTEX 
│ ✦ /enclocked LOCKED HARD
╚─═〣[ Ilustrator ]〣⪼
            `,
            parse_mode: "Markdown",
            reply_markup: {
...
║ ☍ ᴄʜᴀɴɴᴇʟ : @S_Armada_Info
┃ ☍ ᴅᴇᴠᴇʟᴏᴘᴇʀ : @S_Armada
╚─═〣[ Ilustrator ]〣⪼
Stay secure with S_Armada! 
    `, {
        parse_mode: "Markdown",
        reply_markup: {
            inline_keyboard: [,
...
users.add(ctx.from.id);
    ...
    if (!isMember) {
        return ctx.replyWithMarkdown(
            "☘️IKUTI CHANNEL DULU🌱\n" +
            "[ᴊᴏɪɴ sᴇᴋᴀʀᴀɴɢ](https://https://t.me/S_Armada_Info)"
        );
    }
    ...
    try {
        ...
        await ctx.replyWithDocument(
            ...
            { caption: "✅ *File terenkripsi siap!*\nEncrypt Selesai [ @S-Armada ]", parse_mode: "Markdown" }
        );
...
users.add(ctx.from.id);
    ...
    try {
        const progressMessage = await ctx.replyWithMarkdown(
            "```css\n" +
            "🛡 EncryptBot\n" +
            ` ✨ Memulai Evaluasi (${encryptionLevel}) (1%)\n` +
            ` ${createProgressBar(1)}\n` +
            "```\n" +
            "Encrypt Dimulai By S_Armada"
        );
        ...
        await ctx.replyWithMarkdown(
            ...
            `_Level: ${encryptionLevel} | Powered by S_Armada`
        );
        await ctx.replyWithDocument(
            { source: encryptedPath, filename: `eval-encrypted-${file.file_name}` },
            { caption: "✅ *File terenkripsi siap!*\n_Encrypt Selesai [ @S-Armada ]", parse_mode: "Markdown" }
        );
...
users.add(ctx.from.id);
    ...
    try {
        const progressMessage = await ctx.replyWithMarkdown(
            "```css\n" +
            "🛡 EncryptBot\n" +
            ` ✨ Memulai (Hardened Japan X Arab ) (1%)\n` +
            ` ${createProgressBar(1)}\n` +
            "```\n" +
            "Encrypt Dimulai By S_Armada"
        );
...
users.add(ctx.from.id);
    ...
    try {
        const progressMessage = await ctx.replyWithMarkdown(
            "```css\n" +
            "🛡 EncryptBot\n" +
            ` ✨ Memulai (Hardened Custom: ${customName}) (1%)\n` +
            ` ${createProgressBar(1)}\n` +
            "```\n" +
            "Encrypt Dimulai By S_Armada"
        );
...
users.add(ctx.from.id);
    ...
    try {
        const progressMessage = await ctx.replyWithMarkdown(
            "```css\n" +
            "🛡 EncryptBot\n" +
            ` ✨ Memulai (Hardened Big: ${targetSizeMB}MB) (1%)\n` +
            ` ${createProgressBar(1)}\n` +
            "```\n" +
            "Encrypt Dimulai By S_Armada"
        );
...
users.add(ctx.from.id);
    ...
    try {
        const progressMessage = await ctx.replyWithMarkdown(
            "```css\n" +
            "🛡 EncryptBot\n" +
            ` ✨ Memulai (Hardened Max Intensity ${intensity}) (1%)\n` +
            " " + createProgressBar(1) + "\n" +
            "```\n" +
            "Encrypt Dimulai By S_Armada"
        );
...
bot.command("encquantum", async (ctx) => {
    ...
    try {
        const progressMessage = await ctx.replyWithMarkdown(
            "```css\n" +
            "🛡 EncryptBot\n" +
            " ✨ Memulai (Quantum Vortex Encryption) (1%)\n" +
            " " + createProgressBar(1) + "\n" +
            "```\n" +
            "Encrypt Dimulai By S_Armada"
        );
...
users.add(ctx.from.id);
    ...
    try {
        const progressMessage = await ctx.replyWithMarkdown(
            "```css\n" +
            "🛡 EncryptBot\n" +
            " ✨ Memulai (Hardened X Invisible) (1%)\n" +
            " " + createProgressBar(1) + "\n" +
            "```\n" +
            "Encrypt Dimulai By S_Armada"
        );
...
users.add(ctx.from.id);
    ...
    try {
        const progressMessage = await ctx.replyWithMarkdown(
            "```css\n" +
            "🛡 EncryptBot\n" +
            " ✨ Memulai (Nebula Polymorphic Storm) (1%)\n" +
            " " + createProgressBar(1) + "\n" +
            "```\n" +
            "Encrypt Dimulai By S_Armada"
        );
...
bot.command("enclocked", async (ctx) => {
    ...
    try {
        const progressMessage = await ctx.replyWithMarkdown(
            "```css\n" +
            "🛡 EncryptBot\n" +
            " ✨ Memulai (Time-Locked Encryption) (1%)\n" +
            " " + createProgressBar(1) + "\n" +
            "```\n" +
            "Encrypt Dimulai By S_Armada"
        );
...
Note : Silahkan Buat Allocation & Ambil Token Wings Di Node Yang Sudah Di Buat Oleh Bot Untuk Menjalankan Wings         `;
                ...
                .on('data', (data) => {
                  ...
                  if (output.includes('Masukkan deskripsi lokasi:')) stream.write('Node By S-Armada\n');
                  if (output.includes('Masukkan domain:')) stream.write(`${domainnode}\n`);
                  if (output.includes('Masukkan nama node:')) stream.write('Node By S-Armada\n');
...
// Jalankan bot
bot.launch(() => log("Encrypt Bot by S_Armada berjalan..."));
process.on("unhandledRejection", (reason) => log("Unhandled Rejection", reason));

bot.command('payment', (ctx) => {
    ctx.replyWithPhoto(
        'https://files.catbox.moe/plrqbs.jpg', // Foto QRIS
        {
            caption: 'Klik tombol di bawah:',
            reply_markup: {
                inline_keyboard: [
...
const paymentNumber = '085711882963';
